export PROJECT_ID=wise-cycling-321405
...
kubectl config rename-context gke_wise-cycling-321405_asia-south2-a_prod-cluster gke-asia
kubectl config rename-context gke_wise-cycling-321405_us-central1-a_dr-cluster gke-us
...
# Getting the Virtual External IP

gcloud beta container hub ingress describe

# Configure the firewall for Multi Cluster ingress

gcloud compute firewall-rules create todoappfirewall --project $PROJECT_ID --network default  --direction INGRESS --allow tcp:0-65535 --source-ranges 130.211.0.0/22,35.191.0.0/16
